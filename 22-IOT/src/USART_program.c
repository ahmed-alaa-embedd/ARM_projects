...
//{
//#if baud_rate == R115200
//
//	/*	baud rate = 115200		*/
//	USART1 -> BRR = 0x45;
//
//#elif baud_rate ==R9600
//
//	/*	baud rate = 9600		*/
//	USART1 -> BRR = 0x341;
//
...
void MUSART1_voidTransmit(u8 arr[])
{
	...
	while(arr[i] != '\0')
	{
		...
		USART1 -> DR = arr[i];// when use GET_BIT it clear SR
		while((GET_BIT((USART1 -> SR), 6)) == 0);//stuck on this bc i didnt initialize UART prephiral
		i++;
	}
...
u8 MUSART1_u8Receive(u32 Copy_u32timeout)
{
	...
	u8 Loc_u8ReceivedData = 0;
	//add pointer to check on timeOut and then check on this pointer for over time while the data is in Loc_u8ReceivedData

//	Loc_u8ReceivedData = USART1 -> DR;

	// when read SR it delete the RX flag
	while((GET_BIT(USART1 -> SR,5))==0)//stuck here it clear when read
	{
		timeOut++;
		if(timeOut==Copy_u32timeout)
		{
