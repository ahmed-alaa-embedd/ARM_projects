...
//see freertos.org in software timers section i must create a 2 timers with PWM
//u can enable static allocation in rtos new version and solve the problem

/* see this web sites (https://www.freertos.org/a00016.html?_ga=2.67198801.1393209981.1612819641-343342967.1605209258)
 * (https://forums.freertos.org/t/undefined-reference-to-vapplicationidlehook-and-xtaskgetcurrenttaskhandle-while-compiling/8140/2)
 *
 * */
TaskHandle_t IdelTaskHandle = NULL;
TaskHandle_t MotorHandle = NULL;
TaskHandle_t ServoHandle = NULL;
TaskHandle_t vUARTHandle = NULL;
...
void main (void)
{
	//	setbuf(stdout, NULL); //for printf function
...
//	MRCC_voidEnableClock(RCC_APB1,1);		/*Enable clock for TIM3*/
//	MRCC_voidEnableClock(RCC_APB1,2);		/*Enable clock for TIM4*/
...
//	MGPIO_VoidSetPinDirection(GPIOB,PIN9,OUTPUT_SPEED_50MHZ_AFPP);
	/* CONFIGURE PINS AS ALT FUNC > OUTPUT PUSH PULL @ 50HZ FOR PWM for motor*/
//	MGPIO_VoidSetPinDirection(GPIOB,PIN1,OUTPUT_SPEED_50MHZ_AFPP);
...
//	xTaskCreate(vApplicationIdleHook    ,"Idel task"   ,configMINIMAL_STACK_SIZE,NULL ,0,&IdelTaskHandle) ;

	xTaskCreate(vMotor_Handler    ,"motor"   ,configMINIMAL_STACK_SIZE,NULL ,1,&MotorHandle) ;

	xTaskCreate(vServo_Handler    ,"servo"   ,configMINIMAL_STACK_SIZE,NULL ,2,&ServoHandle) ;

	xTaskCreate(vUartRecieve    ,"UART"   ,configMINIMAL_STACK_SIZE,NULL ,3,&vUARTHandle) ;

	xTaskCreate(vCheck    ,"Check"   ,configMINIMAL_STACK_SIZE,NULL ,4,&CheckHandle) ;


//	 TimerHandle_t xTimerCreate(const char * const MyTIM3,
//			 	 	 	 	 	const TickType_t xTimerPeriod3,
//								const UBaseType_t uxAutoReload3,
//								void * const pvTimer3ID,
//								TimerCallbackFunction_t pxCallbackFunction3 );
...
//	 configASSERT( ( uint32_t ) pvParameters == 1UL );
	...
	while (1)
	{
		...
		if(notificationValue>0)
		{
			...
			{
				MGPIO_VoidSetPinValue(GPIOA,PIN0,HIGH);
				//PWM_Angle(45,PWMTIM4);
			}
			else if(GlobalDirection=='L')
			{
				MGPIO_VoidSetPinValue(GPIOA,PIN0,LOW);
				//PWM_Angle(135,PWMTIM4);
			}
		}
	}
