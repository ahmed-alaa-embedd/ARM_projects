...
void MEXTI_VoidInit(u8 copy_EXTI_LINE)
{
...
#error("Wrong External interrupt line")
	...
	SET_BIT(EXTI -> FTSR , copy_EXTI_LINE);
#elif EXTI_SENSE_MODE==RISING_EDGE
	SET_BIT(EXTI -> RTSR , copy_EXTI_LINE);
...
void MEXTI_VoidSetSignalLatch(u8 Copy_U8EXTILine, u8 Copy_U8EXTISenseMode)
{
	switch(Copy_U8EXTISenseMode)
		{
		case RISING		:SET_BIT(EXTI -> RTSR , Copy_U8EXTILine);	/*Rising edge*/ break;
		case FALLING	:SET_BIT(EXTI -> FTSR , Copy_U8EXTILine);			/*FALLING edge*/ break;
