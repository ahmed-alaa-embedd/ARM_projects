...
void MRCC_voidInitSysClock(void)
{
	...
	RCC_CR   = 0x00010000; /*Enable HSE with no bypass (use crystal)*/
	...
	/*	wait until the HSI to be stable	*/
	while(GET_BIT(RCC_CR,1) == 0 );
		...
		RCC_CFGR = 0x00010402; /* HSE clock not divided and HSE oscillator clock selected
							   as PLL input clock and PLL selected as system clock,APB1 is divided by 2 */
		...
		RCC_CR   = 0x01090000; /*Enable HSE with no bypass(use crystal), PLL enable, CSS enable*/
		/*	wait until the PLL to be stable	*/
